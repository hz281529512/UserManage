...
namespace UserManage.AbpCompanyCore
{
    ...
    public class AbpCompanyAppService : UserManageAppServiceBase, IAbpCompanyAppService
    {
        ...
        private readonly IAbpCompanyManager _entityManager;
        ...
        ,IAbpCompanyManager entityManager
        )
        {
...
AbpCompanyEditDto editDto;
		...
		public async Task CreateOrUpdate(CreateOrUpdateAbpCompanyInput input)
		{

			if (input.AbpCompany.Id.HasValue)
			{
				await Update(input.AbpCompany);
			}
			else
			{
				await Create(input.AbpCompany);
			}
		}
		...
		protected virtual async Task Update(AbpCompanyEditDto input)
		{
